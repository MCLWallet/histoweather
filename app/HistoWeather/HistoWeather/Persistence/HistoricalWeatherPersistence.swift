...
//        Coordinates.coordinate.latitude, Coordinates.coordinate.longitude)
	...
	func addHistoricalDaily(from historicalWeather: HistoricalWeather, city: String, country: String) async {
		await context.perform {
			_ = historicalWeather(weather: historicalWeather, city: city, country: country, context: context) }
		context.saveContext()
	}
...
extension HistoricalWeather {
	convenience init(historicalWeather: HistoricalWeather, city: String, country: String, context: NSManagedObjectContext) {
		self.init(context: context)
		self.city = city
		self.country = country
		for i in 0...(historicalWeather.daily.temperature_2m_max.count - 1) {
			addToDay(Day(day: DayEntry(time: convertDate(date: historicalWeather.daily.time[i]),
